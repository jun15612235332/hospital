...
class CKEditor
{
	...
	function replaceAll($className = null)
	{
		...
		if (empty($_config)) {
			...
			if (!empty($className)) {
				...
				$js .= "	if (!classRegex.testES(textarea.className))\n";
				$js .= "		return false;\n";
			}
	...
	function ckeditorPath()
	{
		 ...
		 * For instance, $_SERVER['PHP_SELF'] in a script at the address http://example.com/testES.php/foo.bar
		 * would be /testES.php/foo.bar.
		 */
