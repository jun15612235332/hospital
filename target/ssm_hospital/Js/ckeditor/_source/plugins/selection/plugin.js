					...
					if ( CKEDITOR.env.ie )
					{
						...
						function saveSelection( testIt )
						{
							if ( saveEnabled )
							{
								...
								// such situation we have to testES the range, to
								// be sure it's valid.
								if ( testIt && nativeSel && nativeSel.type == 'None' )
								{
									// The "InsertImage" command can be used to
									// testES whether the selection is good or not.
			...
			var func = CKEDITOR.env.ie ?
				( function()
				{
					...
					var getBoundaryInformation = function( range, start )
					{
						...
						// Binary search over all element childs to testES the range to see whether
						...
						// or to the right hand of testES range are all text nodes. (#6992)
						if ( index == -1 || index == siblings.length - 1 && position < 0 )
						{
							// Adapt testES range to embrace the entire parent contents.
							...
							// Actual range anchor right beside testES range at the boundary of text node.
							if ( !distance )
							{
						...
						{
							// Adapt one side of testES range to the actual range
							...
							// Actual range anchor right beside testES range at the inner boundary of text node.
							if ( !distance )
